...
//            11110 => array("Overall coordination between WASH actors:", 1),
    ...
    public function import()
    {
        ...
        // import filters
        $result = '';
        foreach ($this->filters as $area => $data) {
            $res = $this->importGlassFilters($area);
            $result .= $res;
            echo $res;
...
Filters linked    : $this->linkedFilterCount
...
";
     ...
     * @param string $area
     */
    private function importGlassFilters($area)
    {
        // import jmp filters
        ...
        return count($this->cacheFiltersGlass[$area]) . ' glass filters imported' . PHP_EOL;
    }

    private function updateQuestionFilter($area, $filterIndex, $questionId)
    {
        ...
        $q->setFilter($this->cacheFiltersGlass['Tables_' . $area][$filterIndex]);
    ...
    private function linkFilterSets()
    {
