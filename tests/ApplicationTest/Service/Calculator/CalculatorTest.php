...
class CalculatorTest extends AbstractCalculator
{
    ...
    public function testComputingQuestionnaireIsCorrect()
    {
        ...
        $this->assertEquals($this->answer131->getValuePercent() + $this->answer132->getValuePercent() + $this->answer141->getValuePercent() + $this->answer142->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter1->getId(), $this->questionnaire, $this->part), 'should be the sum of unique children (excluding duplicates via summands)');
        $this->assertEquals($this->answer131->getValuePercent() + $this->answer141->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter11->getId(), $this->questionnaire, $this->part), 'should be the sum of summands');
        $this->assertEquals($this->answer132->getValuePercent() + $this->answer142->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter12->getId(), $this->questionnaire, $this->part), 'should be the sum of summands');
        $this->assertEquals($this->answer131->getValuePercent() + $this->answer132->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter13->getId(), $this->questionnaire, $this->part), 'should be the sum of children');
        $this->assertEquals($this->answer131->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter131->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertEquals($this->answer132->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter132->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertEquals($this->answer141->getValuePercent() + $this->answer142->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter14->getId(), $this->questionnaire, $this->part), 'should be the sum of children');
        $this->assertEquals($this->answer141->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter141->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertEquals($this->answer142->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter142->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertNull($this->getNewCalculator()->computeFilter($this->filter2->getId(), $this->questionnaire, $this->part), 'should be null, when no answer at all');
        $this->assertNull($this->getNewCalculator()->computeFilter($this->filter21->getId(), $this->questionnaire, $this->part), 'should be null, when no answer at all');
        $this->assertEquals($this->answer31->getValuePercent() + $this->answer32->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter3->getId(), $this->questionnaire, $this->part), 'should be the sum of children, when summands have no answer');
        $this->assertEquals($this->answer31->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter31->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertEquals($this->answer32->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter32->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        ...
        $this->assertEquals($this->answer11->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter11->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertEquals($this->answer13->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter13->getId(), $this->questionnaire, $this->part), 'should be the answer, when answer specified');
        $this->assertEquals($this->answer11->getValuePercent() + $this->answer13->getValuePercent() + $this->answer132->getValuePercent() + $this->answer141->getValuePercent() + $this->answer142->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter1->getId(), $this->questionnaire, $this->part), 'should be the sum of children, but with overriden values instead of computed');
        ...
        $this->assertEquals($this->answer141->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter14->getId(), $this->questionnaire, $this->part), 'should be the sum of children, but only for selected part');
        $this->assertEquals($this->answer142->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter14->getId(), $this->questionnaire, $part), 'should be the sum of children, but only for selected part');
        ...
        $this->assertEquals($this->answer21bis->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter2->getId(), $this->questionnaire, $this->part), 'should be the sum of children, including the answer which is specified with alternative filter');
        $this->assertEquals($this->answer21bis->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter21->getId(), $this->questionnaire, $this->part), 'should be the alternative answer, when answer is specified with alternative filter');
        $this->assertEquals($this->answer21bis->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter3->getId(), $this->questionnaire, $this->part), 'should be the sum of summands, when summands have answer');
        ...
        $this->assertEquals($this->answer21bis->getValuePercent() + $this->answer11->getValuePercent() + $this->answer13->getValuePercent() + $this->answer132->getValuePercent() + $this->answer141->getValuePercent(), $this->getNewCalculator()->computeFilter($this->filter3->getId(), $this->questionnaire, $this->part), 'should not sum twice the same filter');
    }
    ...
    public function testComputingFilterIsCorrect()
    {
        ...
        $this->assertNull($service->computeFilter($this->getNewModelWithId('\Application\Model\Filter')->getId(), $this->questionnaire, $this->part), 'empty filter result is always null');
        $this->assertEquals(0.1111, $service->computeFilter($this->highFilter1->getId(), $this->questionnaire, $this->part), 'when only one filter should be equal to that filter');
        $this->assertNull($service->computeFilter($this->highFilter2->getId(), $this->questionnaire, $this->part), 'when only one filter is null should also be null');
        $this->assertEquals(0.111111, $service->computeFilter($this->highFilter3->getId(), $this->questionnaire, $this->part), 'sum all filters');
    }
    ...
    public function testCacheOnFilter()
    {
        ...
        $res1 = $service->computeFilter($this->highFilter3->getId(), $this->questionnaire, $this->part);
        $this->answer131->setValuePercent((12345));
        $res2 = $service->computeFilter($this->highFilter3->getId(), $this->questionnaire, $this->part);
        $this->assertEquals($res1, $res2, 'result should be cached and therefore be the same');

        $service2 = $this->getNewCalculator();
        $res3 = $service2->computeFilter($this->highFilter3->getId(), $this->questionnaire, $this->part);
    ...
    public function testFormulaOverrideNormalResult()
    {
        ...
        $this->assertEquals(0.5, $service->computeFilter($this->filter1->getId(), $this->questionnaire, $this->part));
    }
    ...
    public function testFormulaSyntaxSelf()
    {
        ...
        $this->assertEquals(0.1111, $this->getNewCalculator()->computeFilter($this->filter1->getId(), $this->questionnaire, $this->part), 'should fallback to filter value without any formulas');
        ...
        $this->assertEquals(16, $this->getNewCalculator()->computeFilter($this->filter1->getId(), $this->questionnaire, $this->part), 'should fallback to second formula');
    }
