angular.module('myApp').controller('Browse/ChartCtrl', function ($scope, $location, $http, $timeout, Restangular, $modal)
{
    ...
    $scope.$watch('pointSelected', function (newPointSelected)
    {
        ...
        if (newPointSelected) {
            ...
            var highFilter = $scope.cache($scope.pointSelected.filter, $scope.pointSelected.questionnaire);
            var questionnaire = highFilter.questionnaires[$scope.pointSelected.questionnaire];
            ...
            if (!questionnaire || !questionnaire.filters || questionnaire.filters.length == 0) {
                ...
                Restangular.all('chartFilter').getList(parameters).then(function (data)
                {
                    angular.forEach(data, function(filter){
                        $scope.cache(parameters.filter, parameters.questionnaire, filter, $scope.pointSelected.name);
                    });
    ...
    $scope.getExcludedFilters = function (filtersIdsOnly)
    {
        ...
        angular.forEach($scope.cachedElements, function (hFilter, hFilterId)
        {
    ...
    $scope.getExcludedQuestionnaires = function ()
    {
        ...
        angular.forEach($scope.cachedElements, function (hFilter)
        {
    ...
    $scope.initIgnoredElementsFromUrl = function ()
    {
        ...
        if (excludedFilters && excludedFilters.length) {
            ...
            angular.forEach($scope.cachedElements, function(hFilter, hFilterId) {
                Restangular.one('filter', hFilterId).get({fields: 'officialChildren.officialChildren,officialChildren.officialChildren.officialChildren,officialChildren.officialChildren.officialChildren.officialChildren'}).then(function (hFilterData){
                    angular.forEach(hFilterData.officialChildren, function (filter){
                        $scope.cacheFilters(hFilterData.id, filter, 0);
                    });
                    $scope.isLoading = false;
                });
            });
            ...
            $http.get('/api/chart', { params: {
            ...
            }).success(function (data)
                {
                    data.plotOptions.scatter.dataLabels.formatter = function ()
                    {
                        ...
                        var highFilter = $scope.cache(point[0], point[1]);
                        return $('<span/>').css({color: highFilter.questionnaires[point[1]].ignored ? '#DDD' : this.series.color}).text(this.point.name)[0].outerHTML;
                    };
    ...
    $scope.ignoreAllFiltersForAllHighFilterQuestionnaires = function (hFilterId, bool)
    {
        ...
        angular.forEach($scope.cachedElements[hFilterId].filters, function (filter){
            filter.ignored = bool;
        });
        $scope.refreshChart();
    }
    ...
    $scope.toggleAllForOneQuestionnaire = function (hFilterId, questionnaireId)
    {
        ...
        angular.forEach($scope.cachedElements[hFilterId].questionnaires[questionnaireId], function (questionnaire)
        {
    ...
    {
        angular.forEach($scope.cachedElements[hFilterId].questionnaires, function (questionnaire) {
    ...
    {
        var questionnaire = $scope.cachedElements[highFilterId].questionnaires[questionnaireId];
    ...
    $scope.cache = function (highFilterId, questionnaireId, filter, questionnaireName, ignored, loadName)
    {
        ...
        return $scope.cachedElements[highFilterId];
    }


    $scope.initiateHighFilterCache = function(highFilterId)
    {
        if (!$scope.cachedElements[highFilterId]) {
            $scope.cachedElements[highFilterId] = {};

            Restangular.one('filter', highFilterId).get().then(function(filter){
                $scope.cachedElements[highFilterId].name = filter.name;
            });
        }
    }
    ...
    {
        if (questionnaireId) {
            if (!$scope.cachedElements[highFilterId].questionnaires) {
                $scope.cachedElements[highFilterId].questionnaires = {};
            }
            if (!$scope.cachedElements[highFilterId].questionnaires[questionnaireId]) {
                $scope.cachedElements[highFilterId].questionnaires[questionnaireId] = {id: highFilterId + ':' + questionnaireId};
            }
            if (ignored) {
                $scope.cachedElements[highFilterId].questionnaires[questionnaireId].ignored = ignored;
            }
            if (questionnaireName) {
                $scope.cachedElements[highFilterId].questionnaires[questionnaireId].name = questionnaireName;
                ...
                Restangular.one('questionnaire', questionnaireId).get({fields: 'survey'}).then(function (questionnaire){
                    $scope.cachedElements[highFilterId].questionnaires[questionnaireId].name = questionnaire.survey.code;
                });
            }
        }
    }
    ...
    {
        if (filter) {
            if (!$scope.cachedElements[highFilterId].filters) {
                $scope.cachedElements[highFilterId].filters = [];
            }
            if (!$scope.cachedElements[highFilterId].filters[filter.filter.id]) {
                $scope.cachedElements[highFilterId].filters[filter.filter.id] = {};
            }
            $scope.cachedElements[highFilterId].filters[filter.filter.id].filter = filter.filter;
            if (ignored) {
                $scope.cachedElements[highFilterId].filters[filter.filter.id].ignored = ignored;
            }

            // stock filter values at questionnaire level
            if (filter.values) {
                if (!$scope.cachedElements[highFilterId].questionnaires[questionnaireId].filters) {
                    $scope.cachedElements[highFilterId].questionnaires[questionnaireId].filters = {};
                }
                if (!$scope.cachedElements[highFilterId].questionnaires[questionnaireId].filters[filter.filter.id]) {
                    $scope.cachedElements[highFilterId].questionnaires[questionnaireId].filters[filter.filter.id] = {};
                }
                if (filter.values) {
                    $scope.cachedElements[highFilterId].questionnaires[questionnaireId].filters[filter.filter.id].value = filter.values[0][$scope.part.name];
                }
            }
        }
    }
