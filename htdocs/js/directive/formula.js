...
angular.module('myApp.directives').directive('gimsFormula', function() {
    ...
    return {
        ...
        template:
                ...
                '        <span ng-switch-when="text">{{s.content}}</span>' +
                '        <span class="token" ng-switch-when="self">Ignore this rule</span>' +
                '        <span class="token" ng-switch-when="filterValue" tooltip="Filter: {{s.filter}}, Questionnaire: {{s.questionnaire}}, Part: {{s.part}}"><i class="fa fa-gims-filter"></i> {{s.filter}}<span ng-if="s.questionnaire != \'current\'">, {{s.questionnaire}}</span><span ng-if="s.part != \'current\'">, {{s.part}}</span></span>' +
                '        <span class="token" ng-switch-when="populationValue" tooltip="Questionnaire: {{s.questionnaire}}, Part: {{s.part}}"><i class="fa fa-male"></i> {{s.questionnaire}}<span ng-if="s.part != \'current\'">, {{s.part}}</span></span>' +
                ...
                '        <span class="token" ng-switch-when="regressionFilterValue" tooltip="Filter: {{s.filter}}, Part: {{s.part}}, Year offset: {{s.year}}"><i class="fa fa-gims-filter"></i> {{s.filter}}<span ng-if="s.part != \'current\'">, {{s.part}}</span><span ng-if="s.year != \'0\'">, {{s.year}}</span></span>' +
                ...
                '        <span class="token" ng-switch-when="regressionCumulatedPopulation" tooltip="Part: {{s.part}}"><i class="fa fa-male"></i> {{s.part}}</span>' +
